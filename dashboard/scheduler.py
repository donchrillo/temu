...
from dashboard.job_models import (
    JobType,
    JobStatusEnum,
    JobConfig,
    JobSchedule,
)  # ← KORRIGIERT: job_models statt jobs!
...
class SchedulerService:
    ...
    def add_job(
        ...
        config = JobConfig(
            job_type=job_type,
            schedule=JobSchedule(interval_minutes=interval_minutes, enabled=enabled),
    ...
    async def _run_job(
        ...
        try:
            ...
            if job_type == JobType.SYNC_ORDERS:
                ...
                log_service.log(
                    job_id, job_type.value, "INFO", f"Job Ergebnis: {result}"
                )
                ...
                log_service.log(
                    ...
                    job_type.value,
                    "INFO",
                    "ℹ Inventur-Sync noch nicht implementiert",
                )
                ...
                log_service.log(
                    ...
                    job_type.value,
                    "INFO",
                    "ℹ Rechnungs-Fetch noch nicht implementiert",
                )

            # Erfolg und Abschlussprotokoll
